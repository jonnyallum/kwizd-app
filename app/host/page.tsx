...
import { NeuralHealthMonitor } from '@/components/NeuralHealthMonitor'
...
function HostDashboardContent() {
    ...
    return (
        <AnimatePresence mode="wait">
            {!gameId || loading ? (
                <div key="loading" className="min-h-screen bg-obsidian flex items-center justify-center">
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                ...
                <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="min-h-screen bg-obsidian p-8 relative overflow-hidden"
                >
                    ...
                    <div className="max-w-6xl mx-auto relative z-10">
                        ...
                        <div className="mt-8 flex justify-center">
                            <NeuralHealthMonitor gameId={gameId} />
                        </div>
                    </div>
                </motion.div>
                ...
                <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="min-h-screen bg-obsidian p-4 md:p-8"
                >
                    <div className="max-w-6xl mx-auto">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-6 md:mb-8 bg-white/5 p-4 rounded-2xl border border-white/5">
                            <div className="flex items-center gap-3">
                                <NeuralHealthMonitor gameId={gameId} />
                                <div className="w-px h-8 bg-white/10 mx-2 hidden md:block" />
                                <div className="bg-electric-purple/20 px-4 md:px-6 py-2 md:py-3 rounded-xl flex items-center gap-2">
                                    <ShieldCheck className="w-4 h-4 text-electric-purple" />
                                    <p className="text-white font-bold text-sm md:text-base">
                                        NODE Q{currentQuestionIndex + 1}/{questions.length}
                                    </p>
                                </div>
                        ...
                        <motion.div
                            ...
                            className="glass-card p-6 md:p-12 mb-6 md:mb-8"
                        >
                            <h2 className="text-2xl md:text-4xl font-bold text-white mb-6 md:mb-8">{currentQuestion.text}</h2>

                            {/* Modular Host Display */}
                            {currentQuestion.type === 'numeral' ? (
                                <div className="bg-white/5 border-2 border-white/10 rounded-2xl p-12 text-center">
                                    <p className="text-white/40 text-xs uppercase font-black tracking-[0.5em] mb-4">Numeral Target</p>
                                    <p className={`text-7xl font-black transition-all ${showingAnswer ? 'text-neon-cyan drop-shadow-[0_0_30px_rgba(0,242,255,0.5)]' : 'text-white/20'}`}>
                                        {showingAnswer ? currentQuestion.answer : '????'}
                                    </p>
                                </div>
                            ) : currentQuestion.type === 'sequence' ? (
                                <div className="space-y-3">
                                    <p className="text-white/40 text-xs uppercase font-black tracking-[0.5em] mb-4">Sequence Nodes</p>
                                    {(currentQuestion.options || []).map((option, idx) => (
                                        <div
                                            key={idx}
                                            className={`p-6 rounded-xl border border-white/5 transition-all flex items-center gap-4 ${showingAnswer ? 'bg-green-500/20 border-green-500/30' : 'bg-white/5'}`}
                                        >
                                            <span className="text-neon-cyan font-black italic">#{idx + 1}</span>
                        ...
                        <div className="glass-card p-8 mb-8">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <h3 className="text-2xl font-bold text-white">
                                    Responses ({responses.filter(r => r.question_id === currentQuestion.id).length}/{players.length})
                                </h3>
                            </div>
                                    ...
                                    .map((response, idx) => {
                                        ...
                                        return (
                                            <motion.div
                                                ...
                                                animate={{ x: 0, opacity: 1 }}
                                                key={response.id}
                                                className="flex items-center justify-between bg-white/5 px-6 py-4 rounded-lg"
                                            >
                            ...
                            <AnimatePresence>
                                {[...players].sort((a, b) => b.score - a.score).map((player, idx) => (
                                    <motion.div
                                        ...
                                        className={`glass-card p-6 flex items-center justify-between border-2 transition-all duration-500 ${idx === 0 ? 'border-yellow-400/50 shadow-yellow-400/20' : idx === 1 ? 'border-gray-300/50 shadow-gray-300/20' : idx === 2 ? 'border-orange-400/50 shadow-orange-400/20' : 'border-white/5 opacity-60'
                                            }`}
                                    >
                                        <div className="flex items-center gap-8">
                                            <span className={`text-3xl sm:text-5xl font-black italic tracking-tighter ${idx === 0 ? 'text-yellow-400' : idx === 1 ? 'text-gray-300' : idx === 2 ? 'text-orange-400' : 'text-white/20'}`}>
                                                #{idx + 1}
                                            </span>
